...
MetricsResult compute_metrics(const vector<uint64_t> &latencies_ns,
                              uint64_t total_processed, double duration_sec) {
  ...
  if (!latencies_ns.empty()) {
    ...
    auto it_p50 =
        sorted.begin() + static_cast<size_t>(0.50 * (sorted.size() - 1));
    nth_element(sorted.begin(), it_p50, sorted.end());
    result.p50_us = *it_p50 / 1000;

    auto it_p99 =
        sorted.begin() + static_cast<size_t>(0.99 * (sorted.size() - 1));
