...
OrderBook::OrderBook()
    : bids_(MAX_PRICE_TICKS, 0), asks_(MAX_PRICE_TICKS, 0), best_bid_idx_(-1),
      best_ask_idx_(MAX_PRICE_TICKS) {}
...
vector<Trade> OrderBook::add_order(const Event &e) {
  ...
  if (idx < 0 || idx >= static_cast<int>(MAX_PRICE_TICKS))
    return trades;
  ...
  if (e.side == Side::Buy) {
    ...
    while (qty_remaining > 0 &&
           best_ask_idx_ < static_cast<int>(MAX_PRICE_TICKS) &&
           idx >= best_ask_idx_) {
      uint32_t &ask_qty = asks_[best_ask_idx_];
      uint32_t trade_qty = min(qty_remaining, ask_qty);

      trades.push_back(
          {e.id, static_cast<double>(best_ask_idx_) / 100.0, trade_qty});
      ...
      if (ask_qty == 0) {
        ...
        while (best_ask_idx_ < static_cast<int>(MAX_PRICE_TICKS) &&
               asks_[best_ask_idx_] == 0) {
          best_ask_idx_++;
        }
      }
    }
    ...
    while (qty_remaining > 0 && best_bid_idx_ >= 0 && idx <= best_bid_idx_) {
      ...
      trades.push_back(
          {e.id, static_cast<double>(best_bid_idx_) / 100.0, trade_qty});
...
pair<double, double> OrderBook::get_top_of_book() const {
  ...
  double best_ask = (best_ask_idx_ >= static_cast<int>(MAX_PRICE_TICKS))
                        ? 0.0
                        : (best_ask_idx_ / 100.0);
